...
export function SidebarNav({
  ...
  const projectNavItems = [
    ...
    { id: 'sprint', icon: Timer, label: 'Focus Sprint' },
    { id: 'pacing', icon: Zap, label: 'Pacing Analyzer' },
    { id: 'dialogue', icon: MessageSquare, label: 'Dialogue Lab' },
    { id: 'versions', icon: FileCode, label: 'Draft Versions' },
                  ...
                  <div key={story.id} className="group flex flex-col space-y-1 mb-2">
                    <div className={cn(
                      ...
                      >
                        ...
                        <span className={cn("text-sm font-semibold truncate", activeStoryId === story.id ? "text-white" : "text-muted-foreground")}>
                          {story.title}
                        </span>
                      </div>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48 bg-[#09090b] border-white/5 shadow-2xl">
                          <DropdownMenuItem onClick={() => onAddChapter(story.id)} className="gap-3 cursor-pointer text-white">
                          ...
                          }} className="gap-3 cursor-pointer text-white">
                            <Pencil className="w-4 h-4" /> Rename Manuscript
                          </DropdownMenuItem>
                          <DropdownMenuSeparator className="bg-white/5" />
                          <DropdownMenuItem onClick={() => onDeleteStory(story.id)} className="text-red-500 gap-3 cursor-pointer">
                        ...
                        <div className="grid grid-cols-3 gap-1 mb-2 px-1">
                          {projectNavItems.map((pItem) => (
                             <button
                               ...
                               className={cn(
                                 "flex flex-col items-center justify-center gap-1 p-2 rounded-lg transition-all border shrink-0 overflow-hidden",
                                 activeView === pItem.id
                             ...
                             >
                               ...
                               <span className="text-[7px] font-bold uppercase tracking-tighter truncate w-full text-center">
                                 {pItem.label.split(' ')[0]}
                               </span>
                             </button>
                          ))}
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              activeChapterId === chapter.id 
                                ? "text-primary bg-primary/5 font-bold shadow-sm shadow-primary/5" 
                                : "text-muted-foreground hover:bg-white/[0.02] hover:text-foreground"
                            )}
                          >
        ...
        <DialogContent className="bg-[#09090b] border-white/5 text-white">
          ...
          <div className="py-4">
            <Input 
              ...
              className="bg-white/[0.02] border-white/10 text-white h-12 rounded-xl"
            />
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setRenameDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleRenameSubmit} className="bg-primary text-white hover:bg-primary/90 rounded-xl">Save Changes</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
